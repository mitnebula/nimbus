...
def main():
	...
	for i in range(len(Z2)):
		...
		if i==int(float(sys.argv[1])/T):
			...
			y_prime = Z3-np.mean(Z3)
			yf = scipy.fftpack.fft(y_prime * np.hanning(y_prime.size))
			Z_FFT_array = np.abs(yf[:N//2])
			plt.figure()
			plt.plot(xf, 2.0/N * np.abs(yf[:N//2]), label='Z', linestyle='-')
			y_prime = rout3-np.mean(rout3)
			yf = scipy.fftpack.fft(y_prime * np.hanning(y_prime.size))
			Rout_FFT_array = np.abs(yf[:N//2])
			plt.plot(xf,  2.0/N * np.abs(yf[:N//2]), label='Zout', linestyle='-.')
			plt.xlabel('Frequency (hz)')			
			plt.title('FFT for Zout and Z')
